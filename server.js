...
const gateKeeper = (req, res, next) => {
  ...
  const currentUser = queryString.parse(header);
  console.log(currentUser);
  if (USERS.find((user) => user.userName === currentUser.user) && USERS.find((password) => password.password === currentUser.pass)) {
     req.user = USERS.find((user) => user.userName = currentUser.user);
  }
  next();
}
